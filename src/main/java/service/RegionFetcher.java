...
public class RegionFetcher {
    ...
    private static final Logger log = Logger.getLogger(RegionFetcher.class);
    private final Admin admin;
    private final Table metaTable;
    private final int maxConcurrentRegions;
    private final TableName tableName;
    private final Map<String, List<String>> regionFNHostnameMapping = new HashMap<>();
    private final Set<String> processedRegions = new HashSet<>();
    private final Set<String> allRegions = new HashSet<>();
    private int iteration = 0;

    public RegionFetcher(Connection connection,
                         CompactorConfig config) throws IOException {
        ...
        while (true) {
            ...
            if (results != null && results.length > 0) {
                for (int index = 0; index < results.length; index += 1) {
                    ...
                    String[] tokens = Bytes.toString(result.getRow())
                            .split("\\.");
    ...
    private void refreshRegions() throws IOException {
        allRegions.clear();
        allRegions.addAll(this.admin.getRegions(tableName)
                .stream()
                .map(region -> region.getEncodedName())
                .collect(Collectors.toList()));
    ...
    private Scan getScan(byte[] cf,
                         byte[] column,
                         Optional<PrefixFilter> filterOptional) {
        Scan scan = new Scan();
    ...
    public List<String> getNextBatchOfEncodedRegions(Set<String> inProgressRegions) throws IOException {
        iteration += 1;
        log.info("Fetching regions for this iteration: " + iteration + ", regionsCompactionInProgress: "
                + inProgressRegions.size() + ", regionsCompacted: " + (processedRegions.size()
                - inProgressRegions.size()) + ", allRegions: " + allRegions.size());
        ...
        for (String encodedRegion : allRegions) {
            if (!inProgressRegions.contains(encodedRegion) && encodedRegions.size() < (maxConcurrentRegions
                    - inProgressRegions.size()) && !processedRegions.contains(encodedRegion)) {
                if (!regionFNHostnameMapping.containsKey(encodedRegion)) {
