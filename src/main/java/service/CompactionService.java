...
public class CompactionService {
    ...
    public CompactionService(Connection connection,
    ...
    public void start() throws IOException, InterruptedException {
        ...
        while (true) {
            try {
                ...
                for (String compactingRegion : compactingRegions) {
                    CompactionState compactionState = admin.getCompactionStateForRegion(
                            Bytes.toBytes(compactingRegion));
                    log.info("Compaction state: " + compactingRegion + " - " + compactionState);
                    if (compactionState.equals(CompactionState.MAJOR) || compactionState.equals(
                ...
                log.info(
                        "Received regions in this batch of size: " + regionsBatch.size() + " Regions: " + regionsBatch);
                ...
                for (String encodedRegion : regionsBatch) {
                    ...
                    if (regionInfoMap.containsKey(encodedRegion)) {
                        long lastCompactionTime = admin.getLastMajorCompactionTimestampForRegion(
                                regionInfoMap.get(encodedRegion)
                                        .getRegionName());
                        if (!(boolean) compactorConfig.getConfig(FORCE_COMPACTION)
                                && getTimeAddHours(-1 * RECOMPACT_REGION_HOURS) < lastCompactionTime) {
                            log.info("Skipping compaction for " + encodedRegion + ". Already compacted at " + new Date(
                                    lastCompactionTime) + " which is later than " + new Date(
                                    getTimeAddHours(-1 * RECOMPACT_REGION_HOURS)));
                            continue;
                        }
                    }
        ...
        log.info("compaction complete for regions: " + completedRegions + " in-progress for regions: "
                + compactingRegions);
    }
}
